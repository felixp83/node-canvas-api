...
module.exports = async function generateSolidTemplate(
  ...
  // === Hintergrund in Pastellgelb ===
  ctx.fillStyle = '#FFF5BA';
  ctx.fillRect(0, 0, targetWidth, targetHeight);

  // === Bild als horizontales Oval (zentriert horizontal, 70 px nach oben) ===
  const radiusX = targetWidth * 0.65; // horizontal größer, ragt links/rechts raus
  const radiusY = Math.min(targetWidth, targetHeight) * 0.41; // vertikal wie bisher
  const centerX = targetWidth / 2;

  let centerY;
  if (img && img.height) {
    const minY = 300;
    const maxY = targetHeight - radiusY - 120;
    centerY = Math.min(Math.max(minY + radiusY, targetHeight / 2), maxY) - 95; // Oval 70px nach oben
  } else {
  ...
  ctx.save();
  ctx.beginPath();
  ctx.ellipse(centerX, centerY, radiusX, radiusY, 0, 0, Math.PI * 2); // horizontales Oval
  ctx.closePath();
  ctx.clip();
  ...
  // Weißer Ring um das Oval
  const ringLineWidth = radiusY * 0.08;
  ctx.strokeStyle = '#fff';
  ctx.lineWidth = ringLineWidth;
  ctx.beginPath();
  ctx.ellipse(centerX, centerY, radiusX + ctx.lineWidth / 2, radiusY + ctx.lineWidth / 2, 0, 0, Math.PI * 2);
  ctx.stroke();
  ...
  const ctaY = centerY - radiusY - ctaHeight - 105; // CTA 130 px nach oben
  ...
  // === Overlay-Text (zwischen Oval und Website) ===
  const maxTextWidth = targetWidth * 0.8;
  const minGapToOval = 20;
  const gapAboveUrl = 24;
  const outerRadius = radiusY + ringLineWidth / 2;
  const topLimit = centerY + outerRadius + minGapToOval;
