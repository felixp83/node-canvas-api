...
// Neue Route: Fancy Template
app.post('/fancy', async (req, res) => {
  const imageUrl = req.body.url;
  const website = req.body.website || null;
  let overlayText = req.body.overlay || 'Hello, World!';
  overlayText = overlayText.toUpperCase();
  ...
  try {
    ...
    const canvas = await fancy(img, overlayText, targetWidth, targetHeight, website);

    const filename = `img-fancy-${Date.now()}.png`;
    const savePath = path.join(publicDir, filename);
    const out = fs.createWriteStream(savePath);
    const stream = canvas.createPNGStream();
...
// Route: Center Crop Quadrat Template
app.post('/center-crop-qu', async (req, res) => {
  const imageUrl = req.body.url;
  const website = req.body.website || null;
  ...
  try {
    const img = await loadImage(imageUrl);
    const canvas = await centerCropQu(img, website);

    const filename = `img-center-crop-qu-${Date.now()}.png`;
    const savePath = path.join(publicDir, filename);
    const out = fs.createWriteStream(savePath);
    const stream = canvas.createPNGStream();
...
app.post('/center-crop-qu-zo', async (req, res) => {
  ...
  try {
    const img = await loadImage(imageUrl);
    const canvas = await centerCropQuZ(img);
...
// Route: Center Vignette Template
app.post('/center-qu-v', async (req, res) => {
  const imageUrl = req.body.url;
  const website = req.body.website || null;
  ...
  try {
    const img = await loadImage(imageUrl);
    const canvas = await centerVignette(img, website);

    const filename = `img-center-qu-v-${Date.now()}.png`;
    const savePath = path.join(publicDir, filename);
    const out = fs.createWriteStream(savePath);
    const stream = canvas.createPNGStream();
...
// Route: e_Sanft Template
app.post('/e_sanft', async (req, res) => {
  const imageUrl = req.body.url;
  const website = req.body.website || null;
  let overlayText = req.body.overlay || 'Hello, World!';
  overlayText = overlayText.toUpperCase();
...
// Route: e_Creme Template
app.post('/e_creme', async (req, res) => {
  const imageUrl = req.body.url;
  const website = req.body.website || null;
  let overlayText = req.body.overlay || 'Hello, World!';
  overlayText = overlayText.toUpperCase();
  ...
  try {
    ...
    const canvas = await e_creme(img, overlayText, targetWidth, targetHeight, website);

    const filename = `img-e_creme-${Date.now()}.png`;
    const savePath = path.join(publicDir, filename);
    const out = fs.createWriteStream(savePath);
    const stream = canvas.createPNGStream();
